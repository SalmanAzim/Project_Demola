agGrid.initialiseAgGridWithAngular1(angular);

var app = angular.module('myApp', ['angularCircularNavigation', 'nvd3', 'agGrid']);

app.controller('myController', function ($scope, $timeout) {

    //Custom Variables
    $scope.bulletCurrentValue = 0;
    $scope.bulletPrevValue = 0;

    $scope.screens = {
        floor: true,
        manager: false,
        executive: false,
    }


    // Initialize all screens
    $scope.initializeScreens = function () {
        $scope.screens = {
            floor: false,
            manager: false,
            executive: false,
        }
    };

    $scope.switchType = function (options, clicked) {
        $scope.options.items[1].isActive = false;
        $scope.options.items[2].isActive = false;
        $scope.options.items[0].isActive = false;
        clicked.isActive = true;
        $scope.options.button.cssClass = clicked.cssClass;
        $scope.initializeScreens();
        if (clicked.id === '1') {
            $scope.screens.floor = true;
        } else if (clicked.id === '2') {
            $scope.screens.manager = true;
        } else {
            $scope.screens.executive = true;
        }
    };


    // Round menu button functions
    $scope.options = {
        content: '',
        isOpen: false,
        toggleOnClick: true,
        background: 'green',
        color: 'white',
        size: '',
        button: {
            content: '',
            cssClass: 'fa fa-desktop',
            background: '#FF6F6F',
            color: 'white',
            size: 'big',
            fontsize: 'large'
        },
        items: [
            {
                id: '1',
                cssClass: 'fa fa-industry',
                background: '#2130FF',
                isActive: true,
                onclick: $scope.switchType
            }, {
                id: '2',
                cssClass: 'fa fa-pie-chart',
                background: '#2130FF',
                onclick: $scope.switchType
            }, {
                id: '3',
                cssClass: 'fa fa-money',
                background: '#2130FF',
                onclick: $scope.switchType
            }, {
                empty: true
            }, {
                empty: true
            }, {
                empty: true
            }, {
                empty: true
            }, {
                empty: true
            }, {
                empty: true
            }, {
                empty: true
            }, {
                empty: true
            }
        ]
    };

    //Bar Chart Functions
    $scope.barOptions = {
        chart: {
            type: 'pieChart',
            height: 500,
            x: function (d) { return d.key; },
            y: function (d) { return d.y; },
            showLabels: true,
            duration: 500,
            labelThreshold: 0.01,
            labelSunbeamLayout: true,
            legend: {
                margin: {
                    top: 10,
                    right: 35,
                    bottom: 5,
                    left: 0
                },
                css: {
                    "text-align": "center",
                    "margin": "10px 13px 0px 7px"
                }
            }
        }
    };

    $scope.barData = [
        {
            key: "Phase 1",
            y: 5
        },
        {
            key: "Phase 2",
            y: 2
        },
        {
            key: "Phase 3",
            y: 9
        },
        {
            key: "Phase 4",
            y: 7
        }
    ];

    //Bullet Chart

    $scope.bulletOptions = {
        chart: {
            type: 'bulletChart',
            transitionDuration: 500
        }
    };

    $scope.bulletData = {
        "title": "Production",
        "subtitle": "Units/Day",
        "ranges": [300, 400, 500],
        "measures": [$scope.bulletCurrentValue],
        "markers": [476]
    };

    // Table details 

    var firstNames = ["Anna", "Anni", "aino", "Olivia", "Jaakko", "joona", "Jenna",
        "Amelia", "Jessica", "Jussi", "veeti", "Charlotte", "Mia", "Lucy", "Grace", "Ruby",
        "Vilma", "jasmin", "Freya", "Pinja", "Poppy", "sonja", "Severi"];
    var lastNames = ["Korhonen", "Virtanen", "Mäkinen", "Nieminen", "Mäkelä", "Hämäläinen", "Laine",
        "Niemi", "Jokinen", "Lahtinen", "Cohen", "Ahonen", "Corbin", "Rantanen", "Dalton", "Dane",
        "Donovan", "Easton", "Fisher", "Fletcher"];

    var phases = [
        "phase1", "phase2", "phase3", "phase4"
    ];

    var stat = [
        "Working", "Coffee Break", "In Vacation", "In Sick Leave", "On Duty", "In Meeting"
    ];

    var skills = [
        "Electrician", "Mechanic", "Maintenanace Engineer", "Automation Engineer", "Supervisor", "Manager", "IT Specialist"
    ];

    var addresses = [
        '1197 Thunder Wagon Common, Cataract, RI, 02987-1016, FI, (401) 747-0763',
        '3685 Rocky Glade, Showtucket, NU, X1E-9I0, FI, (867) 371-4215',
        '3235 High Forest, Glen Campbell, MS, 39035-6845, FI, (601) 638-8186',
        '2234 Sleepy Pony Mall , Drain, DC, 20078-4243, FI, (202) 948-3634',
        '2722 Hazy Turnabout, Burnt Cabins, NY, 14120-5642, FI, (917) 604-6597',
        '6686 Lazy Ledge, Two Rock, FI, 92639-3020, FI, (619) 901-9911',
        '2000 Dewy Limits, Wacahoota, NF, A4L-2V9, FI, (709) 065-3959',
        '7710 Noble Pond Avenue, Bolivia, RI, 02931-1842, FI, (401) 865-2160',
        '3452 Sunny Vale, Pyro, ON, M8V-4Z0, FI, (519) 072-8609',
        '4402 Dusty Cove, Many Farms, UT, 84853-8223, FI, (435) 518-0673',
        '5198 Silent Parade, Round Bottom, MD, 21542-9798, FI, (301) 060-7245',
        '8550 Shady Moor, Kitty Fork, CO, 80941-6207, FI, (303) 502-3767',
        '2131 Old Dell, Merry Midnight, AK, 99906-8842, FI, (907) 369-2206',
        '7390 Harvest Crest, Mosquito Crossing, RI, 02957-6116, FI, (401) 463-6348',
        '874 Little Point, Hot Coffee, BC, V3U-2P6, FI, (250) 706-9207',
        '8834 Stony Pioneer Heights, Newlove, OR, 97419-8670, FI, (541) 408-2213',
        '9829 Grand Beach, Flint, UT, 84965-9900, FI, (435) 700-5161',
        '3799 Cozy Blossom Ramp, Ptarmigan, MS, 38715-0313, FI, (769) 740-1526',
        '3254 Silver Island Loop, Maunaloa, DE, 19869-3169, FI, (302) 667-7671',
        '1081 Middle Wood, Taylors Gut Landing, OR, 97266-2873, FI, (541) 357-6310',
        '1137 Umber Trail, Shacktown, NW, X3U-5Y8, FI, (867) 702-6883',
        '9914 Hidden Bank, Wyoming, MO, 64635-9665, FI, (636) 280-4192',
        '7080 Misty Nectar Townline, Coward, AB, T9U-3N4, FI, (403) 623-2838',
        '1184 Wishing Grounds, Vibank, NW, X7D-0V9, FI, (867) 531-2730',
        '126 Easy Pointe, Grandview Beach, KY, 40928-9539, FI, (502) 548-0956',
        '6683 Colonial Street, Swan River, BC, V1A-9I8, FI, (778) 014-4257',
        '960 Gentle Oak Lane, Shakopee, ND, 58618-6277, FI, (701) 327-1219',
        '6918 Cotton Pine Corner, Kenaston, IA, 52165-3975, FI, (515) 906-7427',
        '2368 Burning Woods, Ernfold, NY, 11879-9186, FI, (646) 819-0355',
        '5646 Quiet Shadow Chase, Tiger Tail, IA, 52283-5537, FI, (712) 375-9225',
        '5466 Foggy Mountain Dale, Sweet Home, MT, 59738-0251, FI, (406) 881-1706',
        '5313 Clear Willow Route, Amazon, BC, V0S-2S6, FI, (604) 340-7596',
        '7000 Pleasant Autoroute, Spaceport City, UT, 84749-2448, FI, (435) 154-3360',
        '8359 Quaking Anchor Road, Gross, BC, V9O-0H5, FI, (250) 985-3859',
        '5143 Amber Deer Hollow, New Deal, ND, 58446-0853, FI, (701) 927-0322',
        '6230 Jagged Bear Key, Young, AR, 72337-3811, FI, (501) 805-7239',
        '7207 Heather Vista, Devon, WY, 82520-1771, FI, (307) 358-7092',
        '9416 Red Rise Place, Spraytown, OK, 73809-4766, FI, (580) 867-1973',
        '3770 Golden Horse Diversion, Yelland, IL, 60471-1487, FI, (224) 717-9349',
        '4819 Honey Treasure Park, Alaska, NB, E1U-3I0, FI, (506) 656-9138',
        '6187 Round Front, Land O Lakes, AK, 99873-6403, FI, (907) 853-9063',
        '9218 Crystal Highway, Pickelville, MT, 59847-9299, FI, (406) 076-0024',
        '6737 Bright Quay, Lazy Mountain, KY, 42390-4772, FI, (606) 256-7288',
        '237 Merry Campus, Twentysix, SC, 29330-4909, FI, (864) 945-0157',
        '446 Fallen Gate Rise, Petrolia, SC, 29959-9527, FI, (864) 826-0553',
        '2347 Indian Boulevard, Frisbee, VA, 23797-6458, FI, (703) 656-8445',
        '365 Emerald Grove Line, Level, NC, 28381-1514, FI, (919) 976-7958',
        '1207 Iron Extension, Klickitat, SC, 29197-8571, FI, (803) 535-7888',
        '6770 Cinder Glen, Caronport, OH, 45053-5002, FI, (440) 369-4018',
        '7619 Tawny Carrefour, Senlac, NV, 89529-9876, FI, (775) 901-6433'];


    var columnDefs = [
        {
            headerName: '', width: 30, checkboxSelection: true, suppressSorting: true,
            suppressMenu: true, pinned: true
        },
        {
            headerName: 'Employee',
            children: [
                {
                    headerName: "Name", field: "name",
                    width: 150, pinned: true
                },
                {
                    headerName: "Phase", field: "phase", width: 110, pinned: true
                },
            ]
        },
        {
            headerName: 'Details',
            children: [
                { headerName: "Skill", field: "skill", width: 160, suppressSorting: true },
                { headerName: "Status", field: "status", width: 120 }
            ]
        },
        {
            headerName: 'Contact',
            children: [
                { headerName: "Mobile", field: "mobile", width: 150, filter: 'text' },
                { headerName: "Address", field: "address", width: 500, filter: 'text' }
            ]
        }
    ];

    $scope.gridOptions = {
        columnDefs: columnDefs,
        rowData: createRowData(),
        rowSelection: 'multiple',
        enableColResize: true,
        enableSorting: true,
        enableFilter: true,
        groupHeaders: true,
        rowHeight: 22,
        onModelUpdated: onModelUpdated,
        suppressRowClickSelection: true
    };

    function onModelUpdated() {
        var model = $scope.gridOptions.api.getModel();
        var totalRows = $scope.gridOptions.rowData.length;
        var processedRows = model.getRowCount();
        $scope.rowCount = processedRows.toLocaleString() + ' / ' + totalRows.toLocaleString();
    }

    function createRowData() {
        var rowData = [];

        for (var i = 0; i < 383; i++) {
            rowData.push({
                name: firstNames[i % firstNames.length] + ' ' + lastNames[i % lastNames.length],
                phase: phases[i % phases.length],
                skill: skills[i % skills.length],
                status: stat[i % stat.length],
                mobile: createRandomPhoneNumber(),
                address: addresses[i % addresses.length]
            });
        }

        return rowData;
    }

    function createRandomPhoneNumber() {
        var result = '+358-';
        for (var i = 0; i < 9; i++) {
            result += Math.round(Math.random() * 10);
            if (i === 2 || i === 5 || i === 8) {
                result += ' ';
            }
        }
        return result;
    }

    // line Chart

    $scope.cumOptions = {
        chart: {
            type: 'discreteBarChart',
            height: 450,
            margin: {
                top: 20,
                right: 20,
                bottom: 50,
                left: 55
            },
            x: function (d) { return d.label; },
            y: function (d) { return d.value; },
            showValues: true,
            valueFormat: function (d) {
                return d3.format(',.0f')(d);
            },
            duration: 500,
            xAxis: {
                axisLabel: 'Dates'
            },
            yAxis: {
                axisLabel: 'Extra / lag',
                axisLabelDistance: -10
            }
        }
    };

    $scope.cumData = [
        {
            key: "Cumulative Return",
            values: [
                {
                    "label": "11/05",
                    "value": -290
                },
                {
                    "label": "12/05",
                    "value": 0
                },
                {
                    "label": "13/05",
                    "value": 320
                },
                {
                    "label": "14/05",
                    "value": 196
                },
                {
                    "label": "15/05/2016",
                    "value": 190
                },
                {
                    "label": "16/05",
                    "value": -98
                },
                {
                    "label": "17/05",
                    "value": -130
                },
                {
                    "label": "18/05",
                    "value": -5
                }
            ]
        }
    ];

    //candle chart

    $scope.candleOptions = {
        chart: {
            type: 'candlestickBarChart',
            height: 450,
            margin: {
                top: 20,
                right: 20,
                bottom: 66,
                left: 60
            },
            x: function (d) { return d['date']; },
            y: function (d) { return d['close']; },
            duration: 100,

            xAxis: {
                axisLabel: 'Dates',
                tickFormat: function (d) {
                    return d3.time.format('%x')(new Date(new Date() - (15973 * 86400000) + (d * 86400000)));
                },
                showMaxMin: false
            },

            yAxis: {
                axisLabel: 'Stock Price',
                tickFormat: function (d) {
                    return '$' + d3.format(',.1f')(d);
                },
                showMaxMin: false
            },
            zoom: {
                enabled: true,
                scaleExtent: [1, 10],
                useFixedDomain: false,
                useNiceScale: false,
                horizontalOff: false,
                verticalOff: true,
                unzoomEventType: 'dblclick.zoom'
            }
        }
    };

    $scope.candleData = [{
        values: [
            { "date": 15854, "open": 165.42, "high": 165.8, "low": 164.34, "close": 165.22, "volume": 160363400, "adjusted": 164.35 },
            { "date": 15855, "open": 165.35, "high": 166.59, "low": 165.22, "close": 165.83, "volume": 107793800, "adjusted": 164.96 },
            { "date": 15856, "open": 165.37, "high": 166.31, "low": 163.13, "close": 163.45, "volume": 176850100, "adjusted": 162.59 },
            { "date": 15859, "open": 163.83, "high": 164.46, "low": 162.66, "close": 164.35, "volume": 168390700, "adjusted": 163.48 },
            { "date": 15860, "open": 164.44, "high": 165.1, "low": 162.73, "close": 163.56, "volume": 157631500, "adjusted": 162.7 },
            { "date": 15861, "open": 163.09, "high": 163.42, "low": 161.13, "close": 161.27, "volume": 211737800, "adjusted": 160.42 },
            { "date": 15862, "open": 161.2, "high": 162.74, "low": 160.25, "close": 162.73, "volume": 200225500, "adjusted": 161.87 },
            { "date": 15863, "open": 163.85, "high": 164.95, "low": 163.14, "close": 164.8, "volume": 188337800, "adjusted": 163.93 },
            { "date": 15866, "open": 165.31, "high": 165.4, "low": 164.37, "close": 164.8, "volume": 105667100, "adjusted": 163.93 },
            { "date": 15867, "open": 163.3, "high": 164.54, "low": 162.74, "close": 163.1, "volume": 159505400, "adjusted": 162.24 },
            { "date": 15868, "open": 164.22, "high": 164.39, "low": 161.6, "close": 161.75, "volume": 177361500, "adjusted": 160.9 },
            { "date": 15869, "open": 161.66, "high": 164.5, "low": 161.3, "close": 164.21, "volume": 163587800, "adjusted": 163.35 },
            { "date": 15870, "open": 164.03, "high": 164.67, "low": 162.91, "close": 163.18, "volume": 141197500, "adjusted": 162.32 },
            { "date": 15873, "open": 164.29, "high": 165.22, "low": 163.22, "close": 164.44, "volume": 136295600, "adjusted": 163.57 },
            { "date": 15874, "open": 164.53, "high": 165.99, "low": 164.52, "close": 165.74, "volume": 114695600, "adjusted": 164.87 },
            { "date": 15875, "open": 165.6, "high": 165.89, "low": 163.38, "close": 163.45, "volume": 206149500, "adjusted": 162.59 },
            { "date": 15876, "open": 161.86, "high": 163.47, "low": 158.98, "close": 159.4, "volume": 321255900, "adjusted": 158.56 },
            { "date": 15877, "open": 159.64, "high": 159.76, "low": 157.47, "close": 159.07, "volume": 271956800, "adjusted": 159.07 },
            { "date": 15880, "open": 157.41, "high": 158.43, "low": 155.73, "close": 157.06, "volume": 222329000, "adjusted": 157.06 },
            { "date": 15881, "open": 158.48, "high": 160.1, "low": 157.42, "close": 158.57, "volume": 162262200, "adjusted": 158.57 },
            { "date": 15882, "open": 159.87, "high": 160.5, "low": 159.25, "close": 160.14, "volume": 134848000, "adjusted": 160.14 },
            { "date": 15883, "open": 161.1, "high": 161.82, "low": 160.95, "close": 161.08, "volume": 129483700, "adjusted": 161.08 },
            { "date": 15884, "open": 160.63, "high": 161.4, "low": 159.86, "close": 160.42, "volume": 160402900, "adjusted": 160.42 },
            { "date": 15887, "open": 161.26, "high": 162.48, "low": 161.08, "close": 161.36, "volume": 131954800, "adjusted": 161.36 },
            { "date": 15888, "open": 161.12, "high": 162.3, "low": 160.5, "close": 161.21, "volume": 154863700, "adjusted": 161.21 },
            { "date": 15889, "open": 160.48, "high": 161.77, "low": 160.22, "close": 161.28, "volume": 75216400, "adjusted": 161.28 },
            { "date": 15891, "open": 162.47, "high": 163.08, "low": 161.3, "close": 163.02, "volume": 122416900, "adjusted": 163.02 },
            { "date": 15894, "open": 163.86, "high": 164.39, "low": 163.08, "close": 163.95, "volume": 108092500, "adjusted": 163.95 },
            { "date": 15895, "open": 164.98, "high": 165.33, "low": 164.27, "close": 165.13, "volume": 119298000, "adjusted": 165.13 },
            { "date": 15896, "open": 164.97, "high": 165.75, "low": 164.63, "close": 165.19, "volume": 121410100, "adjusted": 165.19 },
            { "date": 15897, "open": 167.11, "high": 167.61, "low": 165.18, "close": 167.44, "volume": 135592200, "adjusted": 167.44 },
            { "date": 15898, "open": 167.39, "high": 167.93, "low": 167.13, "close": 167.51, "volume": 104212700, "adjusted": 167.51 },
            { "date": 15901, "open": 167.97, "high": 168.39, "low": 167.68, "close": 168.15, "volume": 69450600, "adjusted": 168.15 },
            { "date": 15902, "open": 168.26, "high": 168.36, "low": 167.07, "close": 167.52, "volume": 88702100, "adjusted": 167.52 },
            { "date": 15903, "open": 168.16, "high": 168.48, "low": 167.73, "close": 167.95, "volume": 92873900, "adjusted": 167.95 },
            { "date": 15904, "open": 168.31, "high": 169.27, "low": 168.2, "close": 168.87, "volume": 103620100, "adjusted": 168.87 },
            { "date": 15905, "open": 168.52, "high": 169.23, "low": 168.31, "close": 169.17, "volume": 103831700, "adjusted": 169.17 },
            { "date": 15908, "open": 169.41, "high": 169.74, "low": 169.01, "close": 169.5, "volume": 79428600, "adjusted": 169.5 },
            { "date": 15909, "open": 169.8, "high": 169.83, "low": 169.05, "close": 169.14, "volume": 80829700, "adjusted": 169.14 },
            { "date": 15910, "open": 169.79, "high": 169.86, "low": 168.18, "close": 168.52, "volume": 112914000, "adjusted": 168.52 },
            { "date": 15911, "open": 168.22, "high": 169.08, "low": 167.94, "close": 168.93, "volume": 111088600, "adjusted": 168.93 },
            { "date": 15912, "open": 168.22, "high": 169.16, "low": 167.52, "close": 169.11, "volume": 107814600, "adjusted": 169.11 },
            { "date": 15915, "open": 168.68, "high": 169.06, "low": 168.11, "close": 168.59, "volume": 79695000, "adjusted": 168.59 },
            { "date": 15916, "open": 169.1, "high": 169.28, "low": 168.19, "close": 168.59, "volume": 85209600, "adjusted": 168.59 },
            { "date": 15917, "open": 168.94, "high": 169.85, "low": 168.49, "close": 168.71, "volume": 142388700, "adjusted": 168.71 },
            { "date": 15918, "open": 169.99, "high": 170.81, "low": 169.9, "close": 170.66, "volume": 110438400, "adjusted": 170.66 },
            { "date": 15919, "open": 170.28, "high": 170.97, "low": 170.05, "close": 170.95, "volume": 91116700, "adjusted": 170.95 },
            { "date": 15922, "open": 170.57, "high": 170.96, "low": 170.35, "close": 170.7, "volume": 54072700, "adjusted": 170.7 },
            { "date": 15923, "open": 170.37, "high": 170.74, "low": 169.35, "close": 169.73, "volume": 87495000, "adjusted": 169.73 },
            { "date": 15924, "open": 169.19, "high": 169.43, "low": 168.55, "close": 169.18, "volume": 84854700, "adjusted": 169.18 },
            { "date": 15925, "open": 169.98, "high": 170.18, "low": 168.93, "close": 169.8, "volume": 102181300, "adjusted": 169.8 },
            { "date": 15926, "open": 169.58, "high": 170.1, "low": 168.72, "close": 169.31, "volume": 91757700, "adjusted": 169.31 },
            { "date": 15929, "open": 168.46, "high": 169.31, "low": 168.38, "close": 169.11, "volume": 68593300, "adjusted": 169.11 },
            { "date": 15930, "open": 169.41, "high": 169.9, "low": 168.41, "close": 169.61, "volume": 80806000, "adjusted": 169.61 },
            { "date": 15931, "open": 169.53, "high": 169.8, "low": 168.7, "close": 168.74, "volume": 79829200, "adjusted": 168.74 },
            { "date": 15932, "open": 167.41, "high": 167.43, "low": 166.09, "close": 166.38, "volume": 152931800, "adjusted": 166.38 },
            { "date": 15933, "open": 166.06, "high": 166.63, "low": 165.5, "close": 165.83, "volume": 130868200, "adjusted": 165.83 },
            { "date": 15936, "open": 165.64, "high": 166.21, "low": 164.76, "close": 164.77, "volume": 96437600, "adjusted": 164.77 },
            { "date": 15937, "open": 165.04, "high": 166.2, "low": 164.86, "close": 165.58, "volume": 89294400, "adjusted": 165.58 },
            { "date": 15938, "open": 165.12, "high": 166.03, "low": 164.19, "close": 164.56, "volume": 159530500, "adjusted": 164.56 },
            { "date": 15939, "open": 164.9, "high": 166.3, "low": 164.89, "close": 166.06, "volume": 101471400, "adjusted": 166.06 },
            { "date": 15940, "open": 166.55, "high": 166.83, "low": 165.77, "close": 166.62, "volume": 90888900, "adjusted": 166.62 },
            { "date": 15943, "open": 166.79, "high": 167.3, "low": 165.89, "close": 166, "volume": 89702100, "adjusted": 166 },
            { "date": 15944, "open": 164.36, "high": 166, "low": 163.21, "close": 163.33, "volume": 158619400, "adjusted": 163.33 },
            { "date": 15945, "open": 163.26, "high": 164.49, "low": 163.05, "close": 163.91, "volume": 108113000, "adjusted": 163.91 },
            { "date": 15946, "open": 163.55, "high": 165.04, "low": 163.4, "close": 164.17, "volume": 119200500, "adjusted": 164.17 },
            { "date": 15947, "open": 164.51, "high": 164.53, "low": 163.17, "close": 163.65, "volume": 134560800, "adjusted": 163.65 },
            { "date": 15951, "open": 165.23, "high": 165.58, "low": 163.7, "close": 164.39, "volume": 142322300, "adjusted": 164.39 },
            { "date": 15952, "open": 164.43, "high": 166.03, "low": 164.13, "close": 165.75, "volume": 97304000, "adjusted": 165.75 },
            { "date": 15953, "open": 165.85, "high": 166.4, "low": 165.73, "close": 165.96, "volume": 62930500, "adjusted": 165.96 }
        ]
    }];

    //Random Values generation

    (function update() {
        $timeout(update, 3000 * 1);
        for (i = 0; len = $scope.barData.length, i < len; i++) {
            $scope.barData[i].y = Math.round((Math.random() * 100));
        }
        $scope.bulletCurrentValue = 0;
        for (i = 0; len = $scope.barData.length, i < len; i++) {
            $scope.bulletCurrentValue += $scope.barData[i].y;
        }
        $scope.bulletData = {
            "title": "Production",
            "subtitle": "Units/hour",
            "ranges": [200, 300, 400],
            "measures": [$scope.bulletCurrentValue],
            "markers": [376]
        };
    } ());
});