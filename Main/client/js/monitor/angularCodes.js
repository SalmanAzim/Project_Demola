...
app.controller('MyController', function ($scope, socket, $window, $timeout) {
	...
	var d = new Date();
	// All the values with respect to data are hold by this single json
	$scope.values = {
		overall : {
			phaseProd: {
				data: [
	...
	/**
	 * Socket functions that is used to get the details from the server for the particular parent
	 */
	socket.on('mes_data', function (data) {
		$scope.values[data.id] = data;
	});
